...
def encode(symbols: np.ndarray) -> np.ndarray:
    ...
    paddingSymbols = np.random.default_rng(77).choice(constellation, size=(-len(symbols)) % (SYMBOLS_PER_BLOCK * FRAMES))
...
def get_known_blocks() -> np.ndarray:
    ...
    symbols = get_symbols_from_bitstream(get_non_repeating_bits(BITS_PER_SYMBOL * SYMBOLS_PER_BLOCK * FRAMES, 42), skip_encoding=True)
